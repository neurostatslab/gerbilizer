...
class ConvolutionalTokenizer(Tokenizer):
    ...
    def __init__(self, config: dict, *, create_teacher: bool=True):
        ...
        for in_channels, out_chonnels, kernel_size, stride in zip(
            channels[:-1], channels[1:], tokenizer_config['KERNEL_SIZES'], updated_stride
        ):
            self.convs.append(TokenizerSimpleLayer(in_channels, out_chonnels, kernel_size, stride))
    ...
    def tau(self) -> float:
        ...
        return self.tau_initial + (self.tau_final - self.tau_initial) * self.current_num_steps / self.tau_num_steps
    ...
    def create_teacher(self, config: dict) -> None:
        ...
        for teacher_param, student_param in zip(self.teacher.parameters(), self.parameters()):
    ...
    def step_teacher(self) -> None:
        ...
        tau = self.tau
        for teacher_param, student_param in zip(self.teacher.parameters(), self.parameters()):
            teacher_param.data = (1 - tau) * teacher_param.data + tau * student_param.data.detach().clone()
...
class GerbilizerAttentionNet(nn.Module):
    ...
    @staticmethod
    def __block_mask(length: int, num_unmasked: int, block_length: int) -> torch.BoolTensor:
        ...
        num_start_points = ceil((num_unmasked + 0.05 * length) / block_length)  # Add some extra just in case overlaps result in undermasking
    ...
    def mask_sequence(self, tokens: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.IntTensor]:
        ...
        expanded_tokens = tokens.unsqueeze(1).expand(-1, self.masks_per_sequence, -1, -1)
        unmasked_indices = shuffling[..., :num_unmasked].unsqueeze(-1).expand(-1, -1, -1, d_model)
    ...
    def construct_decoder_input(self, encoded_tokens: torch.FloatTensor, unshuffling: torch.IntTensor) -> torch.FloatTensor:
        ...
        shuffled_tensor = torch.cat([encoded_tokens, expanded_mask_token], dim=-2)  # (bsz * masks_per_sequence, seq_len, d_model)

        # reshaping and expansion in preparation for gather
        unshuffling = unshuffling.reshape(bsz * self.masks_per_sequence, seq_len, 1).expand(-1, -1, d_model)
        return shuffled_tensor.gather(-2, unshuffling)  # returns (bsz * masks_per_sequence, seq_len, d_model)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        encoded_tokens = self.encoder(unmasked_tokens.reshape(-1, num_unmasked, d_model))
        ...
        decode_output = self.decoder(decoder_input).reshape(bsz, masks_per_sequence, -1, d_model)
